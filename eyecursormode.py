...
class EyeTrackingInterface:
    def __init__(self):
        ...
        # Blink modes
        self.interface_mode = "dwell_click"  # Options: "dwell_click", "blink_click"
        self.last_blink_time = None
        self.blink_cooldown = 0.5  # seconds between blinks to avoid multiple triggers
        self.scroll_regions = {"top": 0.2, "bottom": 0.8}  # Screen regions for scrolling
    ...
    def detect_blink(self, eye_images):
        """Detect eye blink from eye images"""
        # Calculate average brightness/darkness of eye regions
        left_eye_open = np.mean(eye_images[:, :, 0]) > 0.3  # Threshold may need adjustment
        right_eye_open = np.mean(eye_images[:, :, 1]) > 0.3
        
        # Consider it a blink if both eyes are closed
        return not (left_eye_open and right_eye_open)
        ...
        while self.control_mode:
            ...
            if result is not None and self.gaze_model is not None:
                ...
                if gaze_position is not None:
                    ...
                    if self.interface_mode == "dwell_click":
                        if y < self.screen_height * self.scroll_regions["top"]:
                            # Top region - prepare for scroll up on dwell
                            if self.handle_dwell_click(x, y):
                                pyautogui.scroll(30)  # Bigger scroll amount
                                cv2.putText(status_display, "DWELL SCROLL UP", (100, 150), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                        elif y > self.screen_height * self.scroll_regions["bottom"]:
                            # Bottom region - prepare for scroll down on dwell
                            if self.handle_dwell_click(x, y):
                                pyautogui.scroll(-30)  # Bigger scroll amount
                                cv2.putText(status_display, "DWELL SCROLL DOWN", (100, 150), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                        else:
                            # Normal region - regular click on dwell
                            if self.handle_dwell_click(x, y):
                                pyautogui.click()
                                cv2.putText(status_display, "DWELL CLICK!", (120, 150), 
            ...
            if key == ord('q'):
                ...
                if self.interface_mode == "dwell_click":
                    self.interface_mode = "blink_click"
                else:
                    self.interface_mode = "dwell_click"
                print(f"Interface mode: {self.interface_mode}")
                
                # Display mode change notification
                mode_display = np.zeros((300, 400, 3), dtype=np.uint8)
                cv2.putText(mode_display, f"Mode: {self.interface_mode}", (50, 150), 
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.imshow("Eye Tracking Status", mode_display)
                cv2.waitKey(1000)
